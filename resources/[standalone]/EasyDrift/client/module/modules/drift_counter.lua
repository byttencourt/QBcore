...
local MoneyEarnedPercentage = 0.05 -- 5% Reward
...
function Modules.DriftCounter.StartChainBreakLoop()
    if not Modules.DriftCounter.ChainLoopStarted then
        ...
        Citizen.CreateThread(function()
            ...
            local percentMoney = 0.01  -- 5% de récompense, par exemple
            local Reward = math.floor(Modules.DriftCounter.CurrentPoints * percentMoney)
            
         --   print("Points : " .. Modules.DriftCounter.CurrentPoints)
         --   print("Récompense en argent : $" .. Reward)
         --   print("Valeur de montantRecompense côté client : $" .. Reward)
        
            -- Envoi de l'événement au serveur
            TriggerServerEvent('EasyDrift:addMoney', Reward)

            -- Afficher la notification au joueur ici
            --SetNotificationTextEntry("STRING")
            --AddTextComponentString("Récompense drift : $" .. Reward)
            local QBCore = exports['qb-core']:GetCoreObject()
            QBCore.Functions.Notify("Recompensa pelo Drift : $" .. Reward,"success")
